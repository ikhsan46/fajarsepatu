...
$q2 ="SELECT COALESCE(SUM(stok),0) as jml FROM tbl_produk p JOIN tbl_kat_produk t ON p.id_kategori = t.id_kategori GROUP BY nm_kategori";
$res2 = mysqli_query($db,$q2);

$q3 = "SELECT COALESCE(SUM(jml_pembayaran),0) as jml FROM tbl_pembayaran";
...
$q5 = "SELECT COALESCE(SUM(stok),0) as jml FROM tbl_produk";
$res5 = mysqli_query($db,$q5);
$dta5 = mysqli_fetch_assoc($res5);

$q6 = "SELECT COALESCE(SUM(total_order),0) as jml FROM tbl_order_offline";
	...
	<div class="row">
		...
		<div class="col-md-6 col-xl-3">
			<div class="mini-stat clearfix bg-purple">
				...
				<div class="mini-stat-info mt-3 float-left">
					<span style="font-size: small;" class="text-white">Total Pendapatan Offline</span>
				</div>
				<div class="clearfix"></div>
				<p class=" mb-0 m-t-10 text-muted">
					<h4 class="counter font-light mt-0 text-white">Rp. <?php echo number_format($dta6['jml']); ?> </h4>
				</p>
			</div>
		</div>
	</div>
